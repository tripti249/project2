...
function initHealthAssistant() {
  ...
  const healthWindow   = document.getElementById('health-window');
  const closeHealthBtn = document.getElementById('close-health-btn');
  const hwCalcBtn      = document.getElementById('hw-calc-btn');
  const hwBackBtn      = document.getElementById('hw-back-btn');
  ...
  hwCalcBtn.addEventListener('click', () => {
    const weight = parseFloat(document.getElementById('hw-weight').value);
    const height = parseFloat(document.getElementById('hw-height').value);
    const age    = parseInt(document.getElementById('hw-age').value);
    const gender = document.getElementById('hw-gender').value;
    ...
    const stats = calculateHealthStats(weight, height, age, gender, activity);
    displayHealthResults(stats);
  });

  hwBackBtn.addEventListener('click', () => {
    hwResults.classList.add('hidden');
    hwForm.classList.remove('hidden');
  });
}
...
function calculateHealthStats(w, h, a, g, act) {
  ...
  return { bmr: Math.round(bmr), tdee, protein, weight: w };
}

function displayHealthResults(stats) {
  document.getElementById('res-bmr').textContent = stats.bmr;
  document.getElementById('res-tdee').textContent = stats.tdee;

  const dietPlanEl = document.getElementById('res-diet-plan');
  const foodListEl = document.getElementById('res-foods');
  ...
  foodListEl.innerHTML = foods.map(f => `<span class="hw-food-chip">${f.name} (${f.type})</span>`).join('');
...
// Re-initialize for health assistant
document.addEventListener('DOMContentLoaded', () => {
  initHealthAssistant();
});
